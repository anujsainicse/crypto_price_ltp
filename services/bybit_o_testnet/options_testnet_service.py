"""Bybit Options Testnet Market Data Service.

Provides real-time options data via single WebSocket connection with dynamic symbol discovery.

Testnet endpoint: wss://stream-testnet.bybit.com/v5/public/option
...
class BybitOptionsTestnetService(BaseService):
    """Service for streaming Bybit Options Testnet data via WebSocket.
    ...
    REST_API_URL = "https://testnet-api.bybit.com/v5/market/instruments-info"
    # Known possible base coins for options on Bybit
    POSSIBLE_BASE_COINS = ['BTC', 'ETH', 'SOL', 'XRP', 'AVAX', 'DOGE', 'LINK', 'MATIC']

    def __init__(self, config: dict):
        """Initialize Bybit Options Testnet Service.

        Args:
            config: Service configuration dictionary
        """
        super().__init__("Bybit-Options-Testnet", config)
        self.config = config

        # API endpoints
        self.ws_url = config.get('websocket_url', 'wss://stream-testnet.bybit.com/v5/public/option')
        ...
        # Redis storage
        self.redis_prefix = config.get('redis_prefix', 'bybit_options_testnet')
        ...
        self.orderbook_redis_prefix = config.get('orderbook_redis_prefix', 'bybit_options_testnet_ob')
    ...
    async def _discover_base_coins(self) -> List[str]:
        """Discover all available base coins with options on Bybit Testnet.
    ...
    async def _fetch_valid_options_symbols(self) -> List[Dict]:
        """Fetch option instruments from Bybit Testnet REST API.
    ...
    async def start(self):
        """Start the Bybit Options Testnet streaming service."""
    ...
    async def stop(self):
        ...
        self._orderbooks.clear()
        self.logger.info("Bybit Options Testnet Service stopped")
...
async def main():
    ...
    config = Settings.load_exchange_config('bybit_options_testnet')
    service_config = config.get('services', {}).get('options', {})

    if not service_config:
        print("No options configuration found in exchanges.yaml for bybit_options_testnet")
        return

    service = BybitOptionsTestnetService(service_config)
    await service.run()
