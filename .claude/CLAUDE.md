...
{
  "ltp": "81895.6",
  "timestamp": "1778058697",
  "original_symbol": "BTCUSDT",
  "candle_timestamp": "1778058652",
  "open": "81854.9",
  "high": "81896.6",
  "low": "81854.9",
  "close": "81895.6",
...
- `timestamp` is the wall-clock time (seconds) when this row was written to Redis — same semantics as every other producer in the repo. Use it for AOE staleness checks.
- `candle_timestamp` is the Bybit-side candle update time (seconds, ms→s converted). Use it when you need the source-of-truth bar time.
...
- A periodic refresher (`_refresh_caches_periodically`, every 45s) re-writes the cached last candle from `self._last_kline` so trade-quiet minutes on illiquid symbols (MNT, HYPE) do not let the LTP key expire.
...
**Bybit Spot Note**: As of v2.9.0, Bybit Spot and Bybit Spot Testnet subscribe to `kline.1.{symbol}` instead of `tickers.{symbol}`. The Redis hash `bybit_spot:{symbol}` now exposes per-candle OHLCV (`open`, `high`, `low`, `close`, `volume`) and `ltp` is sourced from `close`. The 24h fields (`volume_24h`, `high_24h`, `low_24h`, `price_change_percent`) are no longer written — kline does not provide them. Symbol is parsed from the topic string since the kline payload does not include it. The hash exposes both `timestamp` (wall-clock seconds at Redis write time, used for AOE staleness checks) and `candle_timestamp` (Bybit-side candle update time). Because `kline.1` only emits on trade activity, both services run a 45s periodic refresher (`_refresh_caches_periodically`) that re-writes the cached last candle, last orderbook snapshot, and last trades so trade-quiet minutes on illiquid symbols do not let the 60s TTL expire.
